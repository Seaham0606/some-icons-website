...
const { generateIndex } = require('./scripts/generate-changelog-index.cjs');

// Start changelog watcher in development
let changelogWatcher = null;
if (process.env.NODE_ENV !== 'production') {
  try {
    const { watchChangelog } = require('./scripts/watch-changelog.cjs');
    ...
    <html>
      ...
      <body>
        <div class="container">
          ...
          <p>Run: <code>npm run build</code> (from repo root) or <code>npm run build --workspace=apps/icons</code></p>
        </div>
      </body>
    </html>
  `);
